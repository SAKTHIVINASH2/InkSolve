...
# Support both the new google-genai SDK and the old google-generativeai SDK
try:
...
def _call_gemini(prompt: str, img: PILImage.Image) -> str:
    """Call Gemini API with image, supporting both SDK versions."""
    if SDK == "new":
        buf = BytesIO()
...
def analyze_image(img: PILImage.Image, dict_of_vars: dict):
    ...
    for answer in answers:
        if not isinstance(answer, dict):
            continue
        answer['result'] = str(answer.get('result', ''))
        answer['expr'] = str(answer.get('expr', ''))
        answer['assign'] = bool(answer.get('assign', False))

    return [a for a in answers if isinstance(a, dict) and 'expr' in a and 'result' in a]
